...
load('moon_shaped_db_win.mat')

options.ct = lower ( 'mcpcc' );   % select method, "pcc, epcc, mcpcc"
options.sn = upper ( 'L2' );      % select norm type, "L1 or L2"
options.C = abs( 5 );             % penalty coefficient for svm margine training, "a positive scalar"
options.number_centers = int8( 9 );% number of centers," an integer above 1" 
